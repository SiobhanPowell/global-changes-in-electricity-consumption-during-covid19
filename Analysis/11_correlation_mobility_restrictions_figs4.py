...
plt.subplot(211)
plt.title('Initial Period')
plt.boxplot(all_countries_initial, vert=False)
plt.xlim([-1,1])
plt.axvline(0, color='k', alpha=0.3, zorder=10)
plt.xticks([])
plt.yticks(np.arange(1,7), labels=['Retail and Recreation', 'Residences','Workplaces', 'Transit Stations', 'Grocery and Pharmacy', 'Parks'])
plt.subplot(212)
plt.boxplot(all_countries_recovery, vert=False)
plt.xlim([-1,1])
plt.axvline(0, color='k', alpha=0.3, zorder=10)
plt.title('Recovery Period')
plt.xlabel('Correlation with Stringency Index')
plt.yticks(np.arange(1,7), labels=['Retail and Recreation', 'Residences','Workplaces', 'Transit Stations', 'Grocery and Pharmacy', 'Parks'])
plt.tight_layout()
