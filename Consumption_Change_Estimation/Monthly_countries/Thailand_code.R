...
dataset <- read.csv(paste0(data_path,"Thailand.csv"),stringsAsFactors = F)
...
a <- lm(load ~ month + time + cdd, data = dataset %>% filter(year != 2020))
...
abline(v = 96.5, col = 'blue') # beginning of 2020
...
# Export result
write.csv(dataset, paste0(path_result,"Thailand.csv"), row.names = F)
