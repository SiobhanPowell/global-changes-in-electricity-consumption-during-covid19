...
dataset <- read.csv(paste0(data_path,"Kenya.csv"),stringsAsFactors = F)
...
a <- lm(load ~ month + time, data = dataset %>% filter(year != 2020))
...
# Export result
write.csv(dataset, paste0(path_result,"Kenya.csv"), row.names = F)
